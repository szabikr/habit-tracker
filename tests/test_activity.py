...
def test_activity_constructor():
    ...
    assert activity.more_info == None

    activity = Activity(activity_name, activity_date, life_aspect, more_info)
    assert activity.more_info == more_info
...
def test_activity___str__():
    ...
    activity = Activity(activity_name, activity_date, life_aspect, more_info)
    result = str(activity)
    assert result == 'Thu 11 Aug: walk; Health & Fitness | additional details'
...
def test_activity_print():
    ...
    result = activity.print()
    activity_id = activity.id
    assert result == f'{activity_id}; 2022-08-11; walk; Health & Fitness; additional details'
